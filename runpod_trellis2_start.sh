...
WORKFLOWS_REPO="https://github.com/tenitsky/trellis2_template_files.git"
# ============================================================
...
WF_STAGE="$WORKSPACE/.workflows_repo"   # full repo clone (staging)
WF_SUBDIR="workflows"                    # folder inside the repo holding the .json files
...
if [ ! -f "$MARKER" ]; then
  ...
  # ---- Workflows: clone repo to staging, sync only the workflows/ subfolder ----
  mkdir -p "$WF_DIR"
  [ -d "$WF_STAGE/.git" ] || git clone "$WORKFLOWS_REPO" "$WF_STAGE"
  if [ -d "$WF_STAGE/$WF_SUBDIR" ]; then
    cp -f "$WF_STAGE/$WF_SUBDIR"/*.json "$WF_DIR"/ 2>/dev/null || true
    log "Workflows synced from $WF_SUBDIR/ into ComfyUI."
  else
    log "WARNING: '$WF_SUBDIR/' not found in repo - no workflows synced."
  fi
...
# Refresh workflows: pull the staging repo, re-sync the workflows/ subfolder
if [ -d "$WF_STAGE/.git" ]; then
  log "Updating workflows from repo..."
  git -C "$WF_STAGE" pull --ff-only 2>/dev/null || log "workflow pull skipped (offline or diverged)"
  mkdir -p "$WF_DIR"
  [ -d "$WF_STAGE/$WF_SUBDIR" ] && cp -f "$WF_STAGE/$WF_SUBDIR"/*.json "$WF_DIR"/ 2>/dev/null || true
fi
